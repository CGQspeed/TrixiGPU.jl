...
boundary_condition_inflow =
    BoundaryConditionDirichlet(initial_condition_double_mach_reflection)
...
isfile(default_mesh_file) || download(
...
###############################################################################
# run the simulation
sol = solve(ode, SSPRK43(stage_limiter!); ode_default_options()..., callback = callbacks);
