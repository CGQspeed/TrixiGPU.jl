...
function basic_state_baroclinic_instability_longitudinal_velocity(lon, lat, z)
    ...
    const_b =
        ...
        (temperature0 * surface_polar_temperature)
    const_c =
        0.5 * (k + 2) * (surface_equatorial_temperature - surface_polar_temperature) /
    ...
    # In the paper: ̃τ₁, ̃τ₂
    tau1 =
        const_a * lapse_rate / temperature0 * temp1 + const_b * (1 - 2 * scaled_z^2) * temp2
    ...
    big_u =
        gravitational_acceleration / radius_earth *
        k *
        temperature *
        inttau2 *
        (temp3^(k - 1) - temp3^(k + 1))
...
function perturbation_stream_function(lon, lat, z)
    ...
    great_circle_distance_by_a = acos(
        ...
        cos(perturbation_lat) * cos(lat) * cos(lon - perturbation_lon),
    )
...
boundary_conditions =
    Dict(:inside => boundary_condition_slip_wall, :outside => boundary_condition_slip_wall)
