...
function mapping(xi, eta_, zeta_)
    ...
    z =
        zeta +
        1 / 6 * (cos(1.5 * pi * (2 * eta - 3) / 3) * cos(0.5 * pi * (2 * zeta - 3) / 3))

    y = eta + 1 / 6 * (cos(0.5 * pi * (2 * eta - 3) / 3) * cos(2 * pi * (2 * z - 3) / 3))
...
sol = solve(
    ode,
    CarpenterKennedy2N54(williamson_condition = false), #maxiters=1,
