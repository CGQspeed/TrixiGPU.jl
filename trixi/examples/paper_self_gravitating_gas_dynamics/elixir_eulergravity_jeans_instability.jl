...
semi_euler =
    SemidiscretizationHyperbolic(mesh, equations_euler, initial_condition, solver_euler)
...
function Trixi.analyze(
    ...
    e_potential = Trixi.integrate_via_indices(
        ...
        equations_gravity,
        u_gravity,
    ) do u, i, j, element, equations_euler, dg, equations_gravity, u_gravity
        u_euler_local = Trixi.get_node_vars(u_euler, equations_euler, dg, i, j, element)
        u_gravity_local =
            Trixi.get_node_vars(u_gravity, equations_gravity, dg, i, j, element)
