...
function velocity(x, t, vortex_pair::VortexPair)
    ...
    v1 =
        -circulation / (2 * pi) *
        (r_plus / (rc^2 + r_plus^2) * si_plus + r_minus / (rc^2 + r_minus^2) * si_minus)
    v2 =
        circulation / (2 * pi) *
        (r_plus / (rc^2 + r_plus^2) * co_plus + r_minus / (rc^2 + r_minus^2) * co_minus)
...
function (sponge_layer::SpongeLayer)(u, x, t, equations)
    @unpack sponge_layer_min, sponge_layer_max, reference_values, source_terms =
        sponge_layer
...
function source_term_sponge_layer(
    ...
    sponge_layer_min::NTuple{4},
    sponge_layer_max::NTuple{4},
    reference_values,
)
...
function source_term_sponge_layer(
    ...
    s = SVector(
        -alpha_square * (u_prim[1] - reference_values[1]),
        0,
        0,
...
sponge_layer_euler = VortexPairSetup.SpongeLayer(
    sponge_layer_min = (-135 * r0, 115 * r0, -135 * r0, 115 * r0),
    sponge_layer_max = (-115 * r0, 135 * r0, -115 * r0, 135 * r0),
    reference_values = (rho, rho * c0^2 / gamma),
) # (rho0, p0)

boundary_condition_euler =
    VortexPairSetup.BoundaryCondition(rho, (rho * c0^2 / gamma) / (gamma - 1))

semi_euler = SemidiscretizationHyperbolic(
    mesh,
    equations_euler,
    initial_condition_euler,
    solver,
...
sponge_layer_acoustics = VortexPairSetup.SpongeLayer(
    sponge_layer_min = (-135 * r0, 100 * r0, -135 * r0, 100 * r0),
...
callbacks_averaging =
    CallbackSet(summary_callback, alive_callback, averaging_callback, stepsize_callback)
