...
@muladd begin
    @changeprecision Float32 begin
        ...
        varnames(::typeof(cons2prim), ::CompressibleEulerEquations2D) =
            ("rho", "v1", "v2", "p")
        ...
        - polydeg = 5
        """
        function initial_condition_density_wave(
            x,
            t,
            equations::CompressibleEulerEquations2D,
        )
        ...
        @inline function cons2entropy(u, equations::CompressibleEulerEquations2D)
            ...
            w1 =
                (equations.gamma - s) * equations.inv_gamma_minus_one -
                0.5 * rho_p * v_square
            w2 = rho_p * v1
            w3 = rho_p * v2
            w4 = -rho_p
        ...
        @inline function density_pressure(u, equations::CompressibleEulerEquations2D)
            rho, rho_v1, rho_v2, rho_e = u
            rho_times_p =
                (equations.gamma - 1) * (rho * rho_e - 0.5 * (rho_v1^2 + rho_v2^2))
            return rho_times_p
        end
        ...
        @inline function entropy_thermodynamic(
            ...
            p =
                (equations.gamma - 1) *
                (cons[4] - 1 / 2 * (cons[2]^2 + cons[3]^2) / cons[1])
