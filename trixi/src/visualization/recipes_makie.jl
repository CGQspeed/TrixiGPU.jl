...
@muladd begin
    ...
    function global_plotting_triangulation_makie(
        ...
        for element in Base.OneTo(num_elements)
            ...
            trimesh[element] =
                GeometryBasics.normal_mesh(Makie.to_vertices(coordinates), makie_triangles)
        end
    ...
    function convert_PlotData2D_to_mesh_Points(
        ...
        xyz_wireframe =
            GeometryBasics.Point.(
                map(
                    x -> vec(vcat(x, fill(NaN, 1, size(x, 2)))),
                    (x_face, y_face, sol_f),
                )...
            )
    ...
    function iplot(
        ...
        fig[1, 1] = Makie.vgrid!(
            Makie.Label(fig, "Solution field", width = nothing),
            menu,
            Makie.Label(fig, "Solution mesh visible"),
            toggle_solution_mesh,
            Makie.Label(fig, "Mesh visible"),
            toggle_mesh;
            tellheight = false,
            width = 200,
        )
        ...
        plotting_mesh = Makie.@lift(
            global_plotting_triangulation_makie(
                getindex(pd, variable_names[$(menu.selection)]),
            )
        )
        ...
        get_flat_points(wire_points, z_offset) =
        ...
        Makie.Colorbar(
            fig[1, 3],
            limits = Makie.@lift(scaled_extrema($solution_z)),
            colormap = colormap,
        )
    ...
    function iplot(
        ...
        fig_axis_plt =
            iplot!(FigureAndAxes(fig, ax), pd; colormap = colormap, plot_mesh = plot_mesh)
    ...
    function Makie.plot!(myplot::TrixiHeatmap)
        ...
        plotting_mesh =
            global_plotting_triangulation_makie(pds; set_z_coordinate_zero = true)
        ...
        if plot_mesh
            xyz_wireframe =
                convert_PlotData2D_to_mesh_Points(pds; set_z_coordinate_zero = true)
    ...
    function Makie.plot(
        ...
        return Makie.plot(
            PlotData2DTriangulated(sol; solution_variables);
            plot_mesh,
            colormap,
        )
    end
    ...
    function Makie.plot!(
        ...
        axes =
            [Makie.Axis(fig[i, j], xlabel = "x", ylabel = "y") for j = 1:rows, i = 1:cols]
        row_list, col_list = [i for j = 1:rows, i = 1:cols], [j for j = 1:rows, i = 1:cols]
