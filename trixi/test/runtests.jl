...
@time @testset "Trixi.jl tests" begin
    ...
    @time if TRIXI_TEST == "all" || TRIXI_TEST == "mpi"
        ...
        coverage =
            occursin("--code-coverage", cmd) && !occursin("--code-coverage=none", cmd)
        if !(coverage && Sys.iswindows()) && !(coverage && Sys.islinux())
            mpiexec() do cmd
                run(
                    `$cmd -n $TRIXI_MPI_NPROCS $(Base.julia_cmd()) --threads=1 --check-bounds=yes $(abspath("test_mpi.jl"))`,
                )
            end
        end
    end
    ...
    @time if TRIXI_TEST == "all" || TRIXI_TEST == "threaded"
        ...
        run(
            `$(Base.julia_cmd()) --threads=$TRIXI_NTHREADS --check-bounds=yes --code-coverage=none $(abspath("test_threaded.jl"))`,
        )
    end
