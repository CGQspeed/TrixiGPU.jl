...
@testset "DGMulti 2D" begin
    ...
    @trixi_testset "elixir_euler_curved.jl (Quadrilateral elements, GaussSBP, flux differencing)" begin
        @test_trixi_include(
            ...
            l2 = [
                3.4666312082010235e-6,
                3.439277448411873e-6,
                3.439277448308561e-6,
                1.0965598425655705e-5,
            ],
            linf = [
                1.1327280369899384e-5,
                1.1343911921146699e-5,
                1.1343911907157889e-5,
                3.6795826181545976e-5,
            ]
        )
    end
    ...
    @trixi_testset "elixir_euler_weakform.jl (convergence)" begin
        ...
        @test isapprox(
            mean_convergence[:l2],
            [4.243843382379403, 4.128314378833922, 4.128314378397532, 4.081366752807379],
            rtol = 0.05,
        )
    end
    ...
    @trixi_testset "elixir_euler_kelvin_helmholtz_instability.jl" begin
        @test_trixi_include(
            joinpath(EXAMPLES_DIR, "elixir_euler_kelvin_helmholtz_instability.jl"),
    ...
    @trixi_testset "elixir_euler_fdsbp_periodic.jl (arbitrary reference domain)" begin
        @test_trixi_include(
            joinpath(EXAMPLES_DIR, "elixir_euler_fdsbp_periodic.jl"),
            xmin = -200.0,
            xmax = 100.0, #= parameters for reference interval =#
            l2 = [
    ...
    @trixi_testset "elixir_mhd_weak_blast_wave_SBP.jl (Quad)" begin
        ...
        @test_skip @test_trixi_include(
            joinpath(EXAMPLES_DIR, "elixir_mhd_weak_blast_wave_SBP.jl"),
            cells_per_dimension = 4,
            # division by 2.0 corresponds to normalization by the square root of the size of the domain
            l2 =
                [
