...
@testset "TreeMesh MPI" begin
    ...
    @testset "Examples 2D" begin
        ...
        @trixi_testset "elixir_advection_amr_refine_twice.jl" begin
            @test_trixi_include(
                joinpath(EXAMPLES_DIR, "elixir_advection_amr_refine_twice.jl"),
        ...
        @trixi_testset "elixir_advection_amr_coarsen_twice.jl" begin
            @test_trixi_include(
                joinpath(EXAMPLES_DIR, "elixir_advection_amr_coarsen_twice.jl"),
        ...
        if !CI_ON_WINDOWS # see comment on `CI_ON_WINDOWS` in `test/test_mpi.jl`
            @trixi_testset "elixir_hypdiff_lax_friedrichs.jl" begin
                @test_trixi_include(
                    ...
                    linf =
                        [0.0011986956416591976, 0.006423873516411049, 0.006423873516411049]
                )
            end
        end
        ...
        @trixi_testset "elixir_hypdiff_nonperiodic.jl" begin
            @test_trixi_include(
                ...
                linf =
                    [5.5227409524905013e-5, 0.0001454489597927185, 0.00032396328684569653]
            )
        end
