...
@testset "Special elixirs" begin
    @testset "Convergence test" begin
        if !coverage
            ...
            @timed_testset "p4est_2d_dgsem" begin
                ...
                no_refine = @cfunction(
                    (p4est, which_tree, quadrant) -> Cint(0),
                    Cint,
                    (
                        Ptr{Trixi.p4est_t},
                        Ptr{Trixi.p4est_topidx_t},
            ...
            @test_nowarn_mod convergence_test(
                @__MODULE__,
                joinpath(EXAMPLES_DIR, "structured_2d_dgsem", "elixir_advection_basic.jl"),
                2,
                tspan = (0.0, 0.01),
            )
            @test_nowarn_mod convergence_test(
    ...
    @timed_testset "Test linear structure (2D)" begin
        ...
        trixi_include(
            @__MODULE__,
            joinpath(EXAMPLES_DIR, "tree_2d_dgsem", "elixir_hypdiff_lax_friedrichs.jl"),
            tspan = (0.0, 0.0),
            initial_refinement_level = 2,
        )
        A, b = linear_structure(semi)
    ...
    @testset "Test Jacobian of DG (2D)" begin
        ...
        @timed_testset "Compressible Euler equations" begin
            trixi_include(
                @__MODULE__,
                joinpath(EXAMPLES_DIR, "tree_2d_dgsem", "elixir_euler_density_wave.jl"),
                tspan = (0.0, 0.0),
                initial_refinement_level = 1,
            )
        ...
        @timed_testset "MHD" begin
            trixi_include(
                @__MODULE__,
                joinpath(EXAMPLES_DIR, "tree_2d_dgsem", "elixir_mhd_alfven_wave.jl"),
                tspan = (0.0, 0.0),
                initial_refinement_level = 0,
            )
    ...
    @testset "AD using ForwardDiff" begin
        ...
        @timed_testset "elixir_euler_ad.jl" begin
            @test_trixi_include(
                joinpath(examples_dir(), "special_elixirs", "elixir_euler_ad.jl")
            )
        end
    end
end
